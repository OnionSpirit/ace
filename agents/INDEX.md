...
#### Архитектурные инварианты cutex

Этот раздел включает актуальные требования из прежнего отдельного документа по
реализации cutex.

- `cutex` - cooperative userspace mutex: fast path не делает syscall и состоит
  из атомарного `fetch_add` над `_users`.
- Значение `_users == 0` означает свободный mutex; положительное значение
  учитывает владельца и ожидающих. Для счётчика используется знаковый тип, чтобы
  вычитание не приводило к unsigned underflow.
- Slow path не использует busy polling. `capture_future::await_suspend()`
  устанавливает `cutex_router`, а runner передаёт suspended task в MPSC-очередь
  `_waiters`.
- `cutex_control::notify()` извлекает ровно одного waiter-а и возвращает его в
  runner через `runner::reattach()`.
- `cutex::proxy` обеспечивает RAII и запрещает повторный `capture()`/`sync()` без
  `release()`. Proxy нельзя разделять между корутинами.
- Guard объявляется `volatile`, чтобы компилятор не устранил значимый деструктор.
- `sync()` допускает миграцию на runner владельца cutex. В этом режиме нельзя
  полагаться на thread-local state; после критической секции следует явно
  `co_await guard->release()`, чтобы вернуться на исходный runner и восстановить
  roaming state.

#### Race: interrupted queue insertion

Критический сценарий выглядит так:

1. Task A владеет cutex.
2. Task B увеличивает `_users`, не получает fast-path lock и должна перейти в
   `_waiters`.
3. ОС останавливает B между неуспешным `try_lock()` и фактическим enqueue.
4. A освобождает cutex; `notify()` видит ещё пустую очередь.
5. B продолжает работу и enqueue-ится, но без дополнительного уведомления могла
   бы остаться там навсегда.

`pending_notify()` закрывает это окно: если release видит ожидающих по `_users`,
но немедленный `notify()` не находит ноду, отдельная task повторяет уведомление с
кооперативными suspension points. Изменения счётчика, enqueue, release или
маршрутизации обязаны сохранять этот инвариант и проверяться race-тестом.

#### Проверки при изменении cutex

- uncontended capture/release и корректный fast path;
- один waiter и FIFO-пробуждение нескольких waiter-ов;
- interrupted queue insertion и работа `pending_notify()`;
- высокая конкуренция на нескольких runner-ах;
- `capture()` без миграции и `sync()` с миграцией/возвратом;
- RAII release, повторный capture и ручной release для sync-режима;
- cancel ожидающей задачи и взаимодействие с roaming/reattach;
- отсутствие потерь, повторных пробуждений и busy polling.

Корректностные сценарии описываются в `agents/TESTING.md`, а измерения contention
и rescheduling - в `agents/BENCHMARKS.md`. Открытая проблема деструктора sync-proxy
зарегистрирована как B11 в `agents/ISSUES.md`.
...
| `tests/tests.cpp` | `TEST_F` тесты (276 тестов; отключённых нет — `cancel_spawned_with_channel` переоткрыт после фикса B7 в `agents/ISSUES.md`) |
...
roundtrip, schedule throughput. См. инвентарь в `agents/BENCHMARKS.md`.

### Coverage

Цель 95%. Текущее значение: **94.4%** (2226/2357 уникальных строк, gcov,
meson per-test режим; см. `agents/TESTING.md`).
Измерение требует симлинков `tests`/`include` внутри build-каталога (см. `agents/TESTING.md`).
