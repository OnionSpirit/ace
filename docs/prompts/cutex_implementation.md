...
nukes::dynamic::roaming_mpsc_queue<async<>> _waiters {};  // Очередь корутин, ожидающих захвата
...
- Это корутина (возвращает `async<>`), которая:
  1. Пробует `notify()`
...
3. **Интеграция**:
   ...
   - ✅ Использует async<>, runner_pool_t правильно
...
```cpp
ace::async<> safe_task(ace::cutex& mx) {
    ace::croxy proxy(mx);
...
ace::async<> racer(int iterations) {
    ace::croxy prx(cutex);
...
- `async<>` — тип корутины в фреймворке (include/ace/coroutines/context.h)
...
### Функции фреймворка:
- `ace::schedule(async<>&&)` — глобальная планировка (include/ace/core/dispatcher.h)
