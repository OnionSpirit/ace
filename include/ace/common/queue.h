...
namespace ace::common {
    ...
    struct q_node {
        ...
        [[nodiscard]] const T* data() const { return reinterpret_cast<const T*>(storage); }

        void construct(const T& val) { new (storage) T(val); }
        void construct(T&& val) { new (storage) T(std::move(val)); }
    ...
    public:
        ...
        void remove_node(q_node<T>* node) {
            node->destruct();
            unlink(node);
            mempool.free(node);
        }
