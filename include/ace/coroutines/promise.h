 ...
 *
 *  - **`promise_touch_result`** — lifecycle state enum shared between
 *    the runner and the coroutine.
 *  - **`permanent` / `differed`** — policy tags that control whether a
 *    coroutine suspends at creation (`differed`, used by `ace::async`) or
 *    runs immediately (`permanent`, used by `ace::promise`).
 *  - **`promise_return_traits<P, T>`** — CRTP mixin that adds
 *    `return_value()` / `yield_value()` / `return_void()` to a promise type.
 *  - **`promise_traits<T>`** — full promise base that aggregates return
 *    traits, `await_transform()` overloads for all future concepts,
 *    intrusive `operator new` / `operator delete` for control-block
...
namespace ace::coroutines {
     ...
     *
     * @details The runner inspects this value after each call to `awake()` to
    ...
    enum promise_touch_result : uint8_t  {
        ...
        e_finished,             ///< Coroutine reached `co_return` successfully.
     ...
     *
     * @details Used by `ace::promise<T>`. `initial_suspend()` returns
     * `std::suspend_never`, so the coroutine body runs as soon as the
     * return object is constructed.
     */
    struct permanent : promise_rule_traits {
        /// @brief Returns `std::suspend_never` — no suspension at creation.
     ...
     *
     * @details Used by `ace::async<T>`. `initial_suspend()` returns
     * `std::suspend_always`, so the coroutine does not run until it is
     * explicitly scheduled or awaited.
     */
    struct differed : promise_rule_traits {
        /// @brief Returns `std::suspend_always` — suspends on creation.
     ...
     *
     * @details A type satisfies `is_promise_rule` if:
     *  1. It nests `e_promise_rule` (inherits from `promise_rule_traits`).
     *  2. Its static `action()` returns either `std::suspend_never` or
     *     `std::suspend_always`.
     *
    ...
    /**
     * @brief CRTP mixin that provides `return_value()` and `yield_value()`
     *        to a promise type for non-void coroutines.
     *
     * @details This specialization handles coroutines that return a value via
     * `co_return expr` or produce intermediate values via `co_yield expr`.
     *
     * @tparam promiseT  The concrete derived promise type (CRTP).
     * @tparam returnT   The value type returned by `co_return`.
     */
    ...
    struct promise_return_traits {
        ...
        alignas(ACE_BUS_SIZE) returnT _return_value {};                           ///< Storage for the value produced by `co_return`.

        alignas(ACE_BUS_SIZE) promise_touch_result _status { e_inited };          ///< Current lifecycle state.

        /**
         * @brief Called by the coroutine machinery when `co_return expr` is executed.
         * @details Stores the value and transitions status to `e_finished`.
         * @param return_value  Value produced by `co_return`.
         * @return `std::suspend_never` — no suspension after returning.
         */
        ...
        /**
         * @brief Called by the coroutine machinery when `co_yield expr` is executed.
         * @details Stores the intermediate value and transitions status to
         * `e_executed_with_value`, then suspends the coroutine.
         * @param yield_value  Value produced by `co_yield`.
         * @return `std::suspend_always` — suspends after yielding.
         */
    ...
    /**
     * @brief CRTP mixin specialization for `void`-returning coroutines.
     *
     * @details Provides `return_void()` instead of `return_value()`.
     *
     * @tparam promiseT  The concrete derived promise type (CRTP).
     */
    ...
    struct promise_return_traits <promiseT, void> {
        ...
        /**
         * @brief Called by the coroutine machinery when `co_return` (no value) is executed.
         * @return `std::suspend_never` — no suspension.
         */
     ...
     *
     * @details `promise_traits<T>` combines:
     *  - Return-value machinery from `promise_return_traits`.
     *  - `await_transform()` overloads that route `co_await` expressions to
     *    the appropriate future concept (`is_future` vs `is_busy_future`).
     *  - **Intrusive memory layout**: `operator new` allocates a
     *    `control_block` immediately before the promise, enabling external
     *    handles without a separate allocation.
     *  - Optional tracing support via `setup_trace()`.
     *
     * @tparam return_t  The value type returned by `co_return` inside the
     *                   coroutine.  Use `void` for coroutines that do not
     *                   return a value.
     *
     * @par Memory layout
     * @code
    ...
    struct promise_traits : promise_return_traits<promise_traits<return_t>, return_t> {
        ...
        /**
         * @brief Pass-through for `co_await std::suspend_always{}`.
         * @details Clears the busy future pointer and marks status as
         * `e_executed`.
         * @param e  The suspend object.
         * @return The same `std::suspend_always` value.
         */
        ...
        /**
         * @brief Pass-through for `co_await std::suspend_never{}`.
         * @param e  The suspend object.
         * @return The same `std::suspend_never` value.
         */
        ...
        /**
         * @brief `await_transform` for lvalue-ref futures (`is_future` concept).
         * @details Resets `_busy_future` because a regular future takes over
         * forwarding control via the conductor mechanism.
         * @tparam futureT  A type satisfying `ace::common::dispatch::is_future`.
        ...
        /**
         * @brief `await_transform` for rvalue-ref futures (`is_future` concept).
         * @tparam futureT  A type satisfying `ace::common::dispatch::is_future`.
         * @param command   The future to await.
        ...
        /**
         * @brief `await_transform` for lvalue-ref busy futures (`is_busy_future`).
         * @details Sets `_busy_future` so the runner can call `await_ready()`
         * repeatedly before re-queuing the task (active polling).
         * @tparam futureT  A type satisfying `ace::common::dispatch::is_busy_future`.
        ...
        /**
         * @brief `await_transform` for rvalue-ref busy futures (`is_busy_future`).
         * @tparam futureT  A type satisfying `ace::common::dispatch::is_busy_future`.
         * @param future    The busy future to await.
         * @return          An rvalue reference to the future.
         */
        ...
        /**
         * @brief Custom allocator that prepends a `control_block` before the promise.
         * @details Allocates `mem_size + sizeof(control_block)` bytes, constructs
         * a `control_block` at the beginning, then returns a pointer offset by
         * `sizeof(control_block)`.  This enables external handles without a
        ...
        future_handler_ptr_t        _busy_future { nullptr }; ///< Pointer to the currently active busy future, or `nullptr`.
        control_block*              _block  { nullptr };       ///< Pointer to the intrusive control block (set on first `observe()`).
