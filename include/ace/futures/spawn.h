...
namespace ace::futures {
    ...
    class ACE_AWAIT_NODISCARD spawn final : public core::traits::future_traits<spawn<resume_t, rule_t>> {

        typedef async<resume_t, rule_t> async_t;

        async_t                     _task {}; ///< The task to be spawned.
    ...
    public:
         ...
         */
        [[nodiscard]] explicit spawn(async_t&& new_task)
        ...
        bool await_suspend(auto coroutine) {
            ...
            runner_ptr->attach(std::forward<async_t>(_task));
            return false;
        }
...
// NOTE: The short alias ace::spawn is only exposed when ace/ace.h (quick-start
// header) was included before this file — its ACE_H guard switches aliases on.
#ifdef ACE_H
namespace ace {
    /**
     * @brief Short alias for @c ace::futures::spawn.
