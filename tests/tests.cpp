...
TEST_F(future_traits_fixture, is_future_concept) {
    ...
    static_assert(
        ace::core::meta::is_future<ace::timeout>,
        "timeout must satisfy is_future concept"
    );
    static_assert(
        ace::core::meta::is_future<ace::capture_future>,
...
TEST_F(future_traits_fixture, resume_type) {
    ...
    static_assert(std::same_as<resume_type<ace::timeout>, void>);
    SUCCEED();
}
...
TEST_F(promise_traits_fixture, await_transform_future) {
    ...
    static_assert(
        not ace::core::meta::is_busy_future_accurate<ace::timeout, ace::task::promise_type>,
...
TEST_F(cross_mechanic_fixture, cutex_with_timeout) {
    ...
    ace::schedule([&mtx, &result]() -> ace::task {
        co_await ace::timeout(std::chrono::milliseconds(10));
        auto g = ace::guard(mtx);
        auto res = co_await (g.capture() or ace::timeout(std::chrono::milliseconds(5)));
        result << res;
        co_return;
    }());
    ace::run();
